OC.L10N.register(
    "logcleaner",
    {
    "_Delete %n {level} entry_::_Delete %n {level} entries_" : ["Forigu %n {level} eniron","Forigu %n {level} enskribojn"],
    "Delete by error level:": "Forigi laŭ erarnivelo:",
    "no log entries available" : "Neniuj enskribaj enskriboj haveblaj",
    "Time" : "Tempo",
    "IP" : "IP",
    "User" : "Uzanto",
    "App" : "Programo",
    "Method" : "Metodo",
    "URL" : "URL",
    "Reason" : "Kialo",
    "log file cannot be located" : "Registrdosiero ne troveblas",
    "Log entries fetched" : "Ensalutaj enskriboj elprenitaj",
    "Could not fetch log entries" : "Ne povis eltiri enskribajn enskribojn",
    "Could not fetch settings" : "Ne povis eltiri agordojn",
    "Could not set settings" : "Ne povis agordi agordojn",
    "Log entry deleted" : "Ensaluta eniro forigita",
    "_%n log entry successful deleted_::_%n log entries successful deleted_" : ["%n registra eniro sukcesa forigita","%n registroj sukcesaj forigitaj"],
    "How many lines to display" : "Kiom da linioj montri",
    "How many characters should the error reason display" : "Kiom da signoj devas la erara kialo montri",
    "right" : "ĝuste",
    "top" : "supro",
    "which menue to use" : "Kiun menuon uzi",
    "(only effective after the next page refresh)" : "(Nur efika post la sekva paĝo Refreŝigi)",
    "Delete duplicates" : "Forigi duplikatojn",
    "After you have corrected an error from the error log, you can use the trash can symbol on the left of each error line to remove the respective line from the error log." : "Post kiam vi korektis eraron de la erara registro, vi povas uzi la rubon povas simbolo maldekstre de ĉiu erara linio por forigi la respektivan linion el la erara registro.",
    "Simply deleting it does not prevent the error from occurring again!" : "Simple forigi ĝin ne malhelpas la eraron denove okazi!",
    "The weighting of an error is differentiated by the following background colors" : "La pezado de eraro diferencas per la sekvaj fonaj koloroj",
    "Error Level 0: DEBUG: All activity; the most detailed logging." : "Erara Nivelo 0: Elpurigo: Ĉiu Aktiveco; la plej detala arbohakado.",
    "Error Level 1: INFO: Activity such as user logins and file activities, plus warnings, errors, and fatal errors." : "Erara Nivelo 1: Informoj: Aktiveco kiel uzantaj ensalutoj kaj dosieraj agadoj, plus avertoj, eraroj kaj fatalaj eraroj.",
    "Error Level 2: WARN: Operations succeed, but with warnings of potential problems, plus errors and fatal errors." : "Erara Nivelo 2: avertu: operacioj sukcesas, sed kun avertoj pri eblaj problemoj, plus eraroj kaj mortigaj eraroj.",
    "Error Level 3: ERROR: An operation fails, but other services and operations continue, plus fatal errors." : "Erara Nivelo 3: Eraro: Operacio malsukcesas, sed aliaj servoj kaj operacioj daŭras, plus mortigaj eraroj.",
    "Error Level 4: FATAL: The server stops." : "Erara Nivelo 4: Fatala: La servilo ĉesas.",
    "_%n log entry_::_%n log entries_" : ["%n loga eniro","%n enskribaj enskriboj"],
    "Time offset in hours, e.g. due to winter time" : "Tempo kompensas en horoj, ekz. Pro vintra tempo",
    "_Delete %n duplicate_::_Delete %n duplicates_" : ["Forigi %n duplikati","Forigi %n duplikatojn"],
    "This button will delete shown number of duplicates within the error log file" : "Ĉi tiu butono forigos montritan nombron da duplikatoj en la erara protokola dosiero",
    "Empty the log file" : "Malplenigu la protokolan dosieron",
    "Attention!! This function should only be used if you are really sure of what you are doing. This button empties the entire log file, but does not fix any errors." : "Atentu !! Ĉi tiu funkcio devas esti uzata nur se vi vere certas pri tio, kion vi faras. Ĉi tiu butono malplenigas la tutan protokolan dosieron, sed ne riparas erarojn.",
    "Empty the log file now" : "Malplenigu la protokolan dosieron nun",
    "Cancel" : "Nuligi",
    "This button will empty the complete log file" : "Ĉi tiu butono malplenigos la kompletan protokolan dosieron",
    "good" : "Bona",
    "bad" : "Malbona",
    "very bad" : "tre malbone",
    "Hint:" : "Aludo:",
    "deleted" : "forigita",
    "Show messages by this app within the log file" : "Montru mesaĝojn per ĉi tiu app en la protokola dosiero",
    "yes" : "Jes",
    "no" : "Ne",
    "The color at the top of a box indicates the range of the warning." : "La koloro ĉe la supro de skatolo indikas la gamon de la averto.",
    "Okay, I understand and I am sure" : "Bone, mi komprenas kaj mi certas",
    "Amount" : "Kvanto",
    "Apps with the most log entries" : "Aplikoj kun la plej multaj enskribaj enskriboj",
    "Set logging level" : "Agordu logan nivelon",
    "debug" : "Debug",
    "info" : "Informoj",
    "warning" : "AVERTO",
    "error" : "Eraro",
    "fatal" : "Fatala",
    "Automatically delete duplicates every 24 hours" : "Aŭtomate forigi duplikatojn ĉiun 24 horojn",
    "display this settings as accordion or modal" : "Montru ĉi tiujn agordojn kiel akordionon aŭ modalon",
    "accordion" : "Akordiono",
    "modal" : "Modala",
    "Copy to clipboard": "Kopiu al tondujo",
    "Detail": "Detalo",
    "View detail": "Rigardu detalon",
    "Search issue of the log entry on the Internet": "Serĉu temon de la protokolo-eniro en Interreto",
    "Show filters as": "Montru filtrilojn kiel",
"variant 1": "varianto 1",
"variant 2": "varianto 2",
"Filter": "Filtrilo",
"off": "for",
"Filter off": "Filtrilo malŝaltita",
"DEBUG": "ELSENCIGI",
"INFO": "INFO",
"WARN": "AVERTU",
"ERROR": "ERARO",
"FATAL": "FATALA",
"delete all entries of this app": "forigu ĉiujn enskribojn de ĉi tiu programo",
"Delete all entries of the app {name}": "Forigu ĉiujn enskribojn de la aplikaĵo {name}",
"Delete all entries of error level {name}": "Forigu ĉiujn enskribojn de erarnivelo {name}",
"Shall a footer be displayed with useful buttons?": "Ĉu piedlinio estos montrata kun utilaj butonoj?",
"Shall LogCleaner set up the rotation of the log file from a size of 100MB in the configuration file config.php of Nextcloud?": "Ĉu LogCleaner agordu la rotacion de la protokolo-dosiero de grandeco de 100MB en la agorda dosiero config.php de Nextcloud?",
"display minimized view?": "montri minimumigitan vidon?",
"Shall LogCleaner write warnings about the size of the log file to the log file?": "Ĉu LogCleaner skribos avertojn pri la grandeco de la protokolo-dosiero al la protokolo-dosiero?",
"Apparently the PHP function exec() is not available which is why LogCleaner cannot work in turbo mode, but in PHP streaming mode. Changing the php.ini in the directive disable_functions fixes the problem.": "Ŝajne la PHP-funkcio exec() ne disponeblas, tial LogCleaner ne povas funkcii en turba reĝimo, sed en PHP-flua reĝimo. Ŝanĝi la php.ini en la direktivo disable_functions riparas la problemon.",
                 "Shall a log report be sent by email?": "Ĉu registrilo-raporto estos sendita retpoŝte?",
"Log Report Sending Frequency": "Log Raporto Sendanta Ofteco",
"daily": "ĉiutage",
"weekly": "ĉiusemajne",
"monthly": "ĉiumonate",
"To which administrator shall the email be sent?": "Al kiu administranto oni sendos la retpoŝton?",
"Choose an Admin": "Elektu Administranton",
"Test E-Mail": "Testa Retpoŝto",
"Nextcloud Log Summary (%1$s to %2$s)": "Nextcloud Protokolo-Resumo (%1$s al %2$s)",
"Hello %1$s,": "Saluton %1$s,",
"in the period from %1$s to %2$s new log entries were registered.": "en la periodo de %1$s ĝis %2$s novaj protokolaj enskriboj estis registritaj.",
"Distribution by log level:": "Distribuo laŭ lognivelo:",
"Details can be found in the log management of your Nextcloud instance.": "Detaloj troveblas en la protokolo-administrado de via Nextcloud-instanco.",
"This is a test email": "Ĉi tio estas prova retpoŝto",
"Shall a log report be sent by notification?": "Ĉu registro-raporto estos sendita per sciigo?",
"last report: {when}": "lasta raporto: {when}",
"next report: {when}": "sekva raporto: {when}",
"To which administrator shall the notification be sent?": "Al kiu administranto estu sendita la sciigo?",
"Test notification": "Testa sciigo",
"This is a test notification": "Ĉi tio estas testa sciigo",
"Daily Nextcloud Log Summary (%1$s to %2$s)": "Ĉiutaga Nextcloud Protokolo-Resumo (%1$s al %2$s)",
"Weekly Nextcloud Log Summary (%1$s to %2$s)": "Semajna Protokolo-Resumo de Nextcloud (%1$s al %2$s)",
"Monthly Nextcloud Log Summary (%1$s to %2$s)": "Monata Protokolo-Resumo de Nextcloud (%1$s al %2$s)",
                 "if you like {name}, ⭐ rate and/or 💬 comment on {name} in the {where}": "se vi ŝatas {name}, ⭐ taksu kaj/aŭ 💬 komentu {name} en la {where}",
                 "Basic Settings": "Bazaj Agordoj",
"Appearance": "Aspekto",
"Actions": "Agoj",
"Automatic": "Aŭtomata"
},
"nplurals=2; plural=n != 1;");
